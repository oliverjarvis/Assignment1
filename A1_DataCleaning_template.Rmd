...
colnames(demo) <- sapply(1:36, function(x) toupper(colnames(demo)[x]))
colnames(LU) <- sapply(1:12, function(x) toupper(colnames(LU)[x]))
colnames(token) <- sapply(1:8, function(x) toupper(colnames(token)[x]))
...
```{r}
token$VISIT <- str_extract(token$VISIT, regex("\\d"))
LU$VISIT <- str_extract(token$VISIT, regex("\\d"))
```
...
```{r}
demo <- demo %>% select(c(1:4, 7, 9, 14, 24, 33, 22))
LU <- LU %>% select(c(1:3, 8))
token <- token %>% select(c(1:3,4, 6,7))
```
...
```{r}
merged <- merge(demo, LU, all=T)
merged <- merge(merged, token, all=T)
```
...
```{r}
visit1 <- select(merged[merged$VISIT == 1,], SUBJ, ADOS, MULLENRAW, EXPRESSIVELANGRAW, SOCIALIZATION)
colnames(visit1) <- c("SUBJ", sapply(2:5, function(x) paste(colnames(visit1)[x], "1", sep="")))
merged <- merge(merged, visit1, all=T)
...
merged$GENDER <- ifelse(merged$GENDER  == 1, "male", "female")
merged$DIAGNOSIS <- ifelse(merged$DIAGNOSIS == "A", "ASD", "TD")

write_csv(merged, "LangDevASD.csv")
```
...
```{r}
merged %>% group_by(SUBJ) %>% summarise(mMlu=mean(CHI_MLU, na.rm=T)) %>% filter(mMlu > 2.7)
merged %>% filter(VISIT == 1 & CHI_MLU < 1.5 )
merged %>% group_by(SUBJ) %>% summarise(na = sum(is.na(CHI_MLU)), visits = n()) %>% filter(na > 0 | visits < 6)
```
...
```{r}
merged %>% group_by(SUBJ) %>% filter(VISIT == 6) %>% arrange(desc(TOKENS_CHI))
merged %>% group_by(SUBJ) %>% filter(VISIT == 1) %>% arrange(TOKENS_CHI)
```
...
```{r}
#1
merged %>% filter(DIAGNOSIS == "ASD") %>% select(MOT_MLU, CHI_MLU, TOKENS_MOT, TOKENS_CHI)
#2
merged %>% filter(DIAGNOSIS == "ASD") %>% select(MOT_MLU, CHI_MLU, TOKENS_MOT, TOKENS_CHI, TOKENS_CHI, TOKENS_CHI)
...
```{r}
#1.
merged %>% mutate(token_mean = mean(TOKENS_CHI, na.rm=T))
#2
merged %>% group_by(SUBJ) %>% mutate(meanTokens = mean(TOKENS_CHI, na.rm=T))
#3
merged %>% mutate(token_mean = mean(TOKENS_CHI, na.rm=T)) %>% group_by(SUBJ) %>% mutate(meanTokens = mean(TOKENS_CHI, na.rm=T))
